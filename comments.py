...
def plot_series(word,top=1000):
    ...
    for game in bggdata:
        ...
        results.append({'count':gameid, 'id': gameid, 'year': year})
    data_df = pd.DataFrame(results).set_index('id')
    comments_df = analyse_comments(word)
    results = data_df.reset_index().merge(comments_df.reset_index()).set_index('id')
 ...
 #   plt.plot(by_year.loc[1990:].freq_game)
    plt.plot(by_year.loc[1990:].freq_comment)
    return by_year
...
def fit_model(df,username,excluded_cols=None):
    ...
    formula = ols_formula(user, 'rating', excluded_cols)
